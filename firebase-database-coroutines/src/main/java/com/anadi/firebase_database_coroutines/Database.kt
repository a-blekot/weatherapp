package com.anadi.firebase_database_coroutines
...
private suspend fun <T : Any> readReferences(
    ...
    val listener = object : ValueEventListener {
        ...
        override fun onDataChange(snapshot: DataSnapshot) {
            try {
                val data: List<T> = snapshot.children.toHashSet().map {
                    it.getValue(type)!!
                }
 ...
 * @return The persisted object of the type T informed.
 */
suspend fun <T : Any> DatabaseReference.readList(type: Class<T>): List<T> =
    readReferences(this, type)
